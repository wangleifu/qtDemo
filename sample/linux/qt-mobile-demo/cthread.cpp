...
void CThread::set_controller(FlightController *ctr)
{
    this->ctr = ctr;
}
...
void CThread::set_nickName(std::string nick_name)
{
...
void CThread::run()
{
    ...
    } else
    {
        ...
        DATA::Positon        max_delta  = {0.0,0.0,0.0};
        float max;
        if (n ==
        {
        ...
        } else if (n == 4)
        {
            ...
            for(int i = 1; i < d_size;i+=100/freq)
            {
                ...
                m_currentPosition = flight_data[i-1];
                m_lock.unlock();
                usleep(cycleTimeInMs * 1000);
            }
        }
