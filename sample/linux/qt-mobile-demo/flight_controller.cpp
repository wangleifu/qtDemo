...
//          频率： freq       原点： homeGP
void
FlightController::set_X(double n)
{
    X = n;
}
void
...
void
FlightController::goLast(DATA::FlightData& fd)
{
    int                        atHome_count = 0;
...
void FlightController::save_delta(std::string nick_name)
{
...
bool
FlightController::isBigger(DATA::Positon& delta, DATA::Positon& max_delta)
{
...
void FlightController::printData(DATA::FlightData& fd)
{
    Telemetry::GlobalPosition currentBroadcastGP;
...
{
    this->vehicle->obtainCtrlAuthority(1);
    std::cout << " get control authority." << endl;
